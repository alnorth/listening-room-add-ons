function Database() {
	...
	var update = function(tx, currentVersion) {
		...
		if(currentVersion == 2) {
			tx.executeSql('alter table TRACK_PLAY add column user_id text', [], ignore, sqlError);
			tx.executeSql('alter table TRACK_PLAY add column room text', [], ignore, sqlError);
			tx.executeSql('update DB_DATA set version = 3', [], ignore, sqlError);
			currentVersion = 3;
		}
		if(currentVersion == 3) {
			tx.executeSql('alter table TRACK_PLAY add column reported_by_user text', [], ignore, sqlError);
			tx.executeSql('alter table TRACK_PLAY add column reported_by_user_id text', [], ignore, sqlError);
			tx.executeSql('alter table TRACK_PLAY add column send_data int', [], ignore, sqlError);
			tx.executeSql('update TRACK_PLAY set send_data = 0', [], ignore, sqlError);
			tx.executeSql('alter table TRACK_PLAY add column data_track_id int', [], ignore, sqlError);
			tx.executeSql('alter table TRACK_PLAY add column data_artist_id int', [], ignore, sqlError);
			tx.executeSql('alter table TRACK_PLAY add column data_play_id int', [], ignore, sqlError);
			tx.executeSql('update DB_DATA set version = 4', [], ignore, sqlError);
			currentVersion = 4;
		}
		if(currentVersion == 4) {
			tx.executeSql('create table if not exists USER(id text, username text)', [], ignore, sqlError);
			tx.executeSql('update DB_DATA set version = 5', [], ignore, sqlError);
			currentVersion = 5;
		}
	};
	...
	var init = function() {
		webDb.transaction(function(tx) {
			tx.executeSql('create table if not exists TRACK_PLAY ' +
				'(id text, artist text, title text, album text, date int, lastfm_status int, lastfm_url text)', [], ignore, sqlError);
			...
			tx.executeSql('SELECT version FROM DB_DATA', [], function(tx, rs) {
				if(rs.rows.length == 0) {
					...
					update(tx, rs.rows.item(0)["version"]);
				}
			}, sqlError);
			...
			tx.executeSql('delete from TRACK_PLAY where date < ? and lastfm_status <> ? and (send_data = 0 or artist = "" or title = "" or data_play_id is not null)', [yesterday, lastfmStatus.WAITING_FOR_SCROBBLING], ignore, sqlError);
		});
	};
